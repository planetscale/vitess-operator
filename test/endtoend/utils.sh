...
function setupBuildContainerImage() {
  ...
  if [[ -n "${CI:-}" ]] && docker image inspect vitess-operator-pr:latest >/dev/null 2>&1; then
...
function createKindCluster() {
  ...
  kind create cluster --wait 30s --name "${KIND_CLUSTER_NAME}" --image "${KIND_VERSION}"
  echo "Loading docker image into Kind cluster"
  kind load docker-image vitess-operator-pr:latest --name "${KIND_CLUSTER_NAME}"
}
